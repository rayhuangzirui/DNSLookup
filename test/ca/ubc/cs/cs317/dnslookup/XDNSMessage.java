...
public class XDNSMessage {
    ...
    public static final int ZMask =        0b0000000001110000;
     ...
     */
    public XDNSMessage(byte[] recvd, int length) {
        this.length = length;
        buffer = ByteBuffer.wrap(recvd, 0, length);
        buffer.position(DataOffset);
    }
    public XDNSMessage(XDNSMessage other) {
    ...
    public int getZ() {
        int cmd = buffer.getShort(CMDOffset);
        return (cmd & ZMask) >> ZShift;
    }
    public void setZ(int z) {
        int cmd = buffer.getShort(CMDOffset);
        cmd = (cmd & ~ZMask) | ((z << ZShift) & ZMask);
        buffer.putShort(CMDOffset, (short)cmd);
    }
    ...
    public String getName() {
        ...
        if ((len & PtrMask) == PtrMask) {  // This is a pointer
            ...
            if (suffix == null) throw new RuntimeException("suffix should not be null");
    ...
    public ResourceRecord getRR() {
        ...
        if (endpos - startpos != rdatalen) throw new RuntimeException("Resource record length incorrect");
        return rr;
    }
    ...
    public void addResourceRecord(ResourceRecord rr, String section) {
        ...
        if (rr.getRecordType() == RecordType.A && rr.getRecordClass() == RecordClass.IN) {
            ...
            if (rdata.length != 4) throw new RuntimeException("Resource record length incorrect");
            buffer.put(rdata, 0, rdata.length);
        } else if (rr.getRecordType() == RecordType.AAAA && rr.getRecordClass() == RecordClass.IN) {
            InetAddress address = rr.getInetResult();
            rdata = address.getAddress();
            if (rdata.length != 16) throw new RuntimeException("Resource record length incorrect");
            buffer.put(rdata, 0, rdata.length);
    ...
    /**
     * Return the length of the message.  If the message was constructed from a byte array,
     * it will be the length of the bytearray.  If the message was constructed for writing,
     * it will be the current position in the buffer.
     *
     * @return The length of the message
     */
    public int getLength() {
        return length == -1 ? buffer.position() : length;
    }
